...
def _push_clique_to_buffer(clique, pdb_name, layer_ref, buffer):
    buffer.append(
        {
            "size": len(clique),
            "clique": _get_clique_with_names_only(clique),
            "resid": _get_clique_with_resid_only(clique),
            "oldresid": _get_clique_with_old_resid_only(clique),
            "layerinfo": _get_clique_layer_info_only(clique, layer_ref),
            "pdbname": pdb_name,
        }
    )
...
def generate_clique_db(
    proj, cliques_table, conn, out_dir, min_hydrophobic_residues=34, residue_baseline=30
):
    bad_proteins = []
    buffer = []
